...
export type ComposerSlashCommand = "model" | "plan" | "default" | "workflows";
...
export function parseStandaloneComposerSlashCommand(
  ...
  const match = /^\/(plan|default|workflows)\s*$/i.exec(text.trim());
  if (!match) {
    return null;
  }
  const command = match[1]?.toLowerCase();
  if (command === "plan") return "plan";
  if (command === "workflows") return "workflows";
  return "default";
}
